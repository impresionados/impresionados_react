...
export const RegisterForm = () => {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const checkResponse = await fetch(`http://10.102.10.35:8001/users/${encodeURIComponent(email)}`);
      if (checkResponse.ok) {
      ...
      const response = await fetch(`http://10.102.10.35:8001/users/?user_name=${encodeURIComponent(name)}&email=${encodeURIComponent(email)}&password=${encodeURIComponent(password)}&address=${encodeURIComponent(address)}&tlf=${encodeURIComponent(phone)}`, {
        method: 'POST',
